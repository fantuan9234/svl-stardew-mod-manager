...
function getLatestVersion($db, string $platform) {
    $stmt = $db->prepare("SELECT * FROM versions WHERE platform = ? AND is_latest = 1 LIMIT 1");
    $stmt->execute([$platform]);
    $row = $stmt->fetch();
    if (!$row) {
        $fallback = $db->prepare("SELECT * FROM versions WHERE platform = ? ORDER BY created_at DESC LIMIT 1");
        $fallback->execute([$platform]);
        $row = $fallback->fetch();
    }
    return $row ?: null;
}

$latestWin = getLatestVersion($db, 'windows');
$latestMac = getLatestVersion($db, 'macos');
$latestLinux = getLatestVersion($db, 'linux');
            ...
            <div class="relative card text-center py-20 overflow-hidden download-card">
                ...
                <div class="relative z-10">
                    ...
                    <div class="flex flex-col sm:flex-row items-center justify-center gap-3 mb-4">
                        ...
                        <a href="<?php echo h($latestWin['download_url']); ?>" class="btn-primary btn-primary-pulse" onclick="recordDownload('windows')">
                        ...
                        <a href="#" class="btn-primary opacity-50 pointer-events-none">
                            <svg class="w-4 h-4" viewBox="0 0 24 24" fill="currentColor"><path d="M0 3.449L9.75 2.1v9.451H0m10.949-9.602L24 0v11.4H10.949M0 12.6h9.75v9.451L0 20.699M10.949 12.6H24V24l-12.9-1.801"/></svg>
                            <?php echo t('download_btn_windows'); ?>
                        </a>
                        <?php endif; ?>

                        <?php if ($latestMac && $latestMac['download_url']): ?>
                        <a href="<?php echo h($latestMac['download_url']); ?>" class="btn-secondary" onclick="recordDownload('macos')">
                            ...
                            <span><?php echo t('download_btn_macos'); ?></span>
                        </a>
                        <?php else: ?>
                        <a href="#" class="btn-secondary opacity-50 pointer-events-none">
                            <svg class="w-4 h-4" viewBox="0 0 24 24" fill="currentColor"><path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.8-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z"/></svg>
                            <?php echo t('download_btn_macos'); ?>
                        </a>
                        <?php endif; ?>

                        <?php if ($latestLinux && $latestLinux['download_url']): ?>
                        <a href="<?php echo h($latestLinux['download_url']); ?>" class="btn-secondary" onclick="recordDownload('linux')">
                            <svg class="w-4 h-4" viewBox="0 0 24 24" fill="currentColor"><path d="M12.504 0c-.155 0-.315.008-.48.021-4.226.333-3.105 4.807-3.17 6.298-.076 1.092-.3 1.953-1.05 3.02-.885 1.051-2.127 2.75-2.716 4.521-.278.832-.41 1.684-.287 2.489a.424.424 0 00-.11.135c-.26.268-.45.6-.663.839-.199.199-.485.267-.797.4-.313.136-.658.269-.864.68-.09.189-.136.394-.132.602 0 .199.027.4.055.536.058.399.116.728.04.97-.249.68-.28 1.145-.106 1.484.174.334.535.47.94.601.81.2 1.91.135 2.774.6.926.466 1.866.67 2.616.47.526-.116.97-.464 1.208-.946.587-.003 1.23-.269 2.26-.334.699-.058 1.574.267 2.577.2.025.134.063.198.114.333l.003.003c.391.778 1.113 1.368 1.884 1.43.777.07 1.652-.247 2.348-.977.104-.104.186-.22.26-.36.075-.12.14-.28.18-.475.477-.121.91-.427 1.207-.913.3-.493.43-1.13.348-1.943-.016-.148-.05-.293-.098-.435-.023-.078-.05-.16-.08-.238.327-.226.547-.544.674-.927.19-.577.14-1.277-.145-2.063-.226-.621-.556-1.242-.934-1.808-.378-.567-.792-1.082-1.132-1.507-.34-.424-.623-.776-.742-.995-.284-.535-.367-.955-.271-1.397.106-.483.396-.823.69-1.2.294-.379.578-.76.578-1.27 0-.072-.006-.143-.018-.212-.066-.398-.247-.722-.458-1.046-.21-.324-.453-.645-.578-.975-.124-.327-.166-.682-.074-1.066.092-.382.271-.742.524-1.08.506-.678 1.279-1.266 2.225-1.632a.424.424 0 00.066-.033c.028-.015.06-.03.084-.05a.424.424 0 00.078-.074c.022-.028.04-.06.054-.093a.424.424 0 00.032-.103c.008-.036.01-.074.006-.112a.424.424 0 00-.024-.11.424.424 0 00-.014-.033C18.488.422 15.373 0 12.504 0z"/></svg>
                            <span><?php echo t('download_btn_linux'); ?></span>
                        </a>
                        <?php else: ?>
                        <a href="#" class="btn-secondary opacity-50 pointer-events-none">
                            <svg class="w-4 h-4" viewBox="0 0 24 24" fill="currentColor"><path d="M12.504 0c-.155 0-.315.008-.48.021-4.226.333-3.105 4.807-3.17 6.298-.076 1.092-.3 1.953-1.05 3.02-.885 1.051-2.127 2.75-2.716 4.521-.278.832-.41 1.684-.287 2.489a.424.424 0 00-.11.135c-.26.268-.45.6-.663.839-.199.199-.485.267-.797.4-.313.136-.658.269-.864.68-.09.189-.136.394-.132.602 0 .199.027.4.055.536.058.399.116.728.04.97-.249.68-.28 1.145-.106 1.484.174.334.535.47.94.601.81.2 1.91.135 2.774.6.926.466 1.866.67 2.616.47.526-.116.97-.464 1.208-.946.587-.003 1.23-.269 2.26-.334.699-.058 1.574.267 2.577.2.025.134.063.198.114.333l.003.003c.391.778 1.113 1.368 1.884 1.43.777.07 1.652-.247 2.348-.977.104-.104.186-.22.26-.36.075-.12.14-.28.18-.475.477-.121.91-.427 1.207-.913.3-.493.43-1.13.348-1.943-.016-.148-.05-.293-.098-.435-.023-.078-.05-.16-.08-.238.327-.226.547-.544.674-.927.19-.577.14-1.277-.145-2.063-.226-.621-.556-1.242-.934-1.808-.378-.567-.792-1.082-1.132-1.507-.34-.424-.623-.776-.742-.995-.284-.535-.367-.955-.271-1.397.106-.483.396-.823.69-1.2.294-.379.578-.76.578-1.27 0-.072-.006-.143-.018-.212-.066-.398-.247-.722-.458-1.046-.21-.324-.453-.645-.578-.975-.124-.327-.166-.682-.074-1.066.092-.382.271-.742.524-1.08.506-.678 1.279-1.266 2.225-1.632a.424.424 0 00.066-.033c.028-.015.06-.03.084-.05a.424.424 0 00.078-.074c.022-.028.04-.06.054-.093a.424.424 0 00.032-.103c.008-.036.01-.074.006-.112a.424.424 0 00-.024-.11.424.424 0 00-.014-.033C18.488.422 15.373 0 12.504 0z"/></svg>
                            <?php echo t('download_btn_linux'); ?>
                        </a>
                        <?php endif; ?>
                    </div>
                    <?php
                    $altLinks = [];
                    if ($latestWin && !empty($latestWin['download_url_alt'])) {
                        $altLinks[] = ['url' => $latestWin['download_url_alt'], 'label' => $latestWin['download_label_alt'] ?: 'Windows 便携版', 'platform' => 'windows'];
                    }
                    if ($latestMac && !empty($latestMac['download_url_alt'])) {
                        $altLinks[] = ['url' => $latestMac['download_url_alt'], 'label' => $latestMac['download_label_alt'] ?: 'macOS 备选', 'platform' => 'macos'];
                    }
                    if ($latestLinux && !empty($latestLinux['download_url_alt'])) {
                        $altLinks[] = ['url' => $latestLinux['download_url_alt'], 'label' => $latestLinux['download_label_alt'] ?: 'Linux AppImage', 'platform' => 'linux'];
                    }
                    if (!empty($altLinks)):
                    ?>
                    <div class="flex flex-wrap items-center justify-center gap-3 text-sm" style="color: var(--text-secondary);">
                        <span>其他版本：</span>
                        <?php foreach ($altLinks as $alt): ?>
                        <a href="<?php echo h($alt['url']); ?>" class="announce-link" onclick="recordDownload('<?php echo h($alt['platform']); ?>')"><?php echo h($alt['label']); ?></a>
                        <?php endforeach; ?>
                    </div>
                    <?php endif; ?>
